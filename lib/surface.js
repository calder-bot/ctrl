 ...
 * @param {Function} opts.colorFn     — custom (out, value, min, max) → Color, default depthColorTo
...
export function createSurfaceMesh(V, opts = {}) {
  ...
  const colorFn = opts.colorFn || depthColorTo;
  ...
  _applyHeights(geo, V, xRange, yRange, heightScale, colorFn);
  ...
  mesh.userData._colorFn = colorFn;
  return mesh;
}
...
export function updateSurfaceMesh(mesh, V) {
  ...
  const { _xRange, _yRange, _heightScale, _colorFn } = mesh.userData;
  _applyHeights(geo, V, _xRange, _yRange, _heightScale, _colorFn);
}

/** Internal: set vertex Y-positions and colors from V. */
function _applyHeights(geo, V, xRange, yRange, heightScale, colorFn) {
  const pos = geo.attributes.position;
  ...
  for (let i = 0; i < count; i++) {
    ...
    colorFn(tmpColor, heights[i], vMin, vMax);
